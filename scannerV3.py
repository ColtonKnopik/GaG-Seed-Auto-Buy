...
def verify_color(x, y):
    ...
    if newColor != color:
        return True
    #Colors to buy
    if color in (legendary, mythical, divine):
        return True

    #Colors to skip
    elif color in (common, uncommon, rare):
        return False
...
def buy_seed(x, y):
    ...
    if (buy_y := find_buy_button_y()) is not None:
        pyautogui.moveTo(700, buy_y)
        subprocess.run([ahk_path, script_path]) #Click
        time.sleep(0.5)
        close_seed(700, buy_y)
...
def find_buy_button_y():
    ...
    buy_button_low_y = 660
    buy_button_high_y = 868
...
def restock_listener():
    ...
    time.sleep(2)
    pyautogui.moveTo(restock_pos)

    while True:
        color = pyautogui.pixel(*restock_pos)
        if color == restock_color:
            print("Restock available!")
            scan_loop()
...
def scan_loop():
    while True:
        init_menu()
        result = scan_position(1086, 822, TOTAL_SEEDS - 2)
        if result:
